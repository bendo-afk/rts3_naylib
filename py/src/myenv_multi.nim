...
proc getActionMask(unitId: int): array[24, bool] {.exportpy.} =
  if worldEnv.units[unitId].isDead:
    result.fill(false)
    result[6] = true
    result[21] = true
    return
...
proc getObsAlly(unitId: int): array[15, float32] {.exportpy.} =
...
proc getObsEnemy(unitId: int): array[15, float32] {.exportpy.} =
  let u = worldEnv.units[unitId]
  result[0] = u.isDead().float32
  
  if result[0] == 0:
    let vis = u.vision.visibleState
    var targetPos = if vis == visVisible: u.move.pos else: u.vision.lastPosition
    
    var absPos = targetPos / maxPosX
    if vis != visVisible and u.vision.lastPosition.x < -10000.0:
      absPos = Vector2(x: 0, y: 0) 

    let obs = fillUnitObs(u, absPos)
    for i in 0..13: result[i+1] = obs[i]
...
proc getObsUnitsMap(isAlly: bool): array[3, array[maxX + 1, array[maxY + 1, float32]]] {.exportpy.} =
  # result[0]: 味方 (1.0)
  # result[1]: 敵の現在地 (1.0)
  # result[2]: 敵の最終確認位置 (0.5)

  let
    mySide = if isAlly: Team.Ally else: Team.Enemy
  
  for u in worldEnv.units:
    let tile = worldEnv.map.pos2tile(u.move.pos)

    if u.team == mySide:
      result[0][tile.x][tile.y] += 0.1
    else:
      if u.vision.visibleState == visVisible:
        ...
        if u.vision.lastPosition.x > -10000.0:
          ...
          if lastTile.x in 0..maxX and lastTile.y in 0..maxY:
            result[2][lastTile.x][lastTile.y] += 0.1
